...
def city_input():
    ...
    print('Chicago: 1, New York: 2, Washington: 3')
    print(' ')
    city = input('Please choose the city whose stats you would like to see: ')
    city = city.lower()
...
def get_time():
    ...
    '''
    period = input('\nFilter the data by month and day of the month, day of the week or select all? Type "no" for no period filter.\n')
    period = period.lower()

    while True: 
        if period == "month":
            while True:
                day_month = input("\nType 'YES' or 'NO' if you wish to filter the data by day of the month\n").lower()
                if day_month == "no":
...
def month_info(m):
    if m == 'month':
        month = input('\nChoose a month! January, February, March, April, May, or June')
...
def disp_raw_data(df):
    ...
    while True:
        ...
        see_data = input("\n Would you like to see five more rows of the data? Please write 'yes' or 'no' \n").lower()
...
def main():
    ...
    # all the conclusions
    stats_funcs_list = [month_freq, day_freq, hour_freq, ride_duration, common_trip, stations_freq, bike_users, birth_years, gender_data]
